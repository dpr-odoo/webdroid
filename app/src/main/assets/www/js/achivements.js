$(document).ready(function(){
	var Achievement = dashboard.extend({
		db: WDDatabase.pool("user_achievements"),
		init: function(){
			this.bindData();
			var self = this;
				...
				_.each(rows, function(row){
					var item = '<tr><td>';
						...
						item += '<p>'+row.summary+'</p>';
			  			item += '<a href="javascript:void(0);" data-id="'+row._id+'" class="achievement_remove text-danger pdl16 pull-right"><span class="fa fa-remove fa-2x text-danger"></span> Remove</a>';
			  			item += '</div></div></td></tr>';
			  		items += item + "\n";
				});
				$("#table_body").html(items);
				$(".achievement_remove").each(function(i){
					$(this).on("click", function(){
						var id = $(this).attr("data-id");
						self.deleteAchievement(id);
					});
				});
			});
		},
		...
		createAchievement: function(){
			...
			if(this.validForm()){
				this.db.create(this.getFormData()).then(function(newId){
					if(newId>0){
						self.wd.get("notify").call("showToast",{"message":"Achievement added"});
						self.bindData();
					}	
				});
			}
		...
		validForm: function(){
			...
			if(title == ""){
				this.notify("Provide Achievement Title");
				return false;
			}
			if(summary == ""){
				this.notify("Provide some details about achievement");
				return false;
			}
			return true;
		},
		...
		deleteAchievement: function(lang_id){
			var self = this;
			...
			.then(function(res){
				if(res.key == "yes"){
					self.db.delete([["_id", "=", lang_id]]).then(function(res){
						if(res > 0){
							self.notify("Achievement deleted !");
							self.bindData();
						}
					});
				}
